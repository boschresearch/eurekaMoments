...
# This source code is from pytorch-image-models/timm
...
class CosineTemperatureScheduler(object):
    """
    Inspiration from
    ...
    def _get_lr(self, t):
        ...
        t_min = self.t_min * gamma
        lr_max_values = [gamma * self.t_max]

        lrs = [
            t_min + 0.5 * (lr_max - t_min) * (1 + math.cos(math.pi * t_curr / t_i)) for lr_max in lr_max_values
        ]
...
def get_temperature_schedule(args):
    if args.temperature_annealing:
        if args.temperature_schedule == 'linear':
            temperatures = np.linspace(args.end_temperature, args.start_temperature, args.epochs)[::-1]
        elif args.temperature_schedule == 'constant':
            temperatures = np.ones((args.epochs,)) * args.start_temperature
        elif args.temperature_schedule =='cosine':
            temperatures = CosineTemperatureScheduler(t_initial=args.epochs,
                                                      t_max=args.start_temperature,
                                                      t_min=args.end_temperature, decay_rate=args.decay_rate_temp)
            temperatures = np.array([temperatures.get_epoch_values(ep) for ep in range(args.epochs)]).squeeze()
        elif args.temperature_schedule =='half_cosine':
            temperatures = CosineTemperatureScheduler(t_initial=int(args.epochs/2),
                                                      t_max=args.start_temperature,
                                                      t_min=args.end_temperature, decay_rate=args.decay_rate_temp)
            temperatures = np.array([temperatures.get_epoch_values(ep) for ep in range(int(args.epochs/2))]).squeeze()
            temperatures2 = np.ones_like(temperatures) * args.end_temperature
            temperatures = np.concatenate([temperatures, temperatures2], axis=0)
    else:
        temperatures = None
    return temperatures
