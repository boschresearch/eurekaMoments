...
class Multi_Hidden_Mlp(nn.Module):
    """ MLP as used in Vision Transformer, MLP-Mixer and related networks
    """
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., hidden_dims=1):
        super().__init__()
        ...
        if hidden_dims > 1:
            for i in range(hidden_dims - 1):
                fc1 = nn.Linear(hidden_features, hidden_features)
                layers.append(fc1)
                act = act_layer()
                layers.append(act)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 return_attention=False, return_z=False,
                 args=None):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,
                              return_attention=return_attention, return_z=return_z, args=args)
        ...
        self.return_attention = return_attention
        self.args = args
        if not block_nr is None:
            self.block_nr = block_nr + 1
    ...
    def forward(self, x):
        ...
        att_out = self.attn(self.norm1(data['x']), scale)
        att = att_out['x']

        if self.return_attention:
            data['attn_out'] = att_out['attn']

        if self.args.linprobe_after_residuals:
            att_out['z'] = att_out['z'] + data['x'].reshape(data['x'].shape[0], data['x'].shape[1], -1,
                                                            att_out['z'].shape[-1]).permute(0, 2, 1, 3)
        data['x'] = data['x'] + self.drop_path(att)
        data['x'] = data['x'] + self.drop_path(self.mlp(self.norm2(data['x'])))

        for k in att_out.keys():
            if k == 'x':
                continue
            elif k in data.keys():
                data[k] = torch.cat([data[k], att_out[k]], dim=1)
            else:
                data[k] = att_out[k]
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 use_head=True,
                 return_z=[False] * 12, 
                 args=None, pos_embed_type='learnable', **kwargs):
        """
        Args:
        ...
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=args.layer_norm_eps)
        ...
        if pos_embed_type == 'learnable':
            ...
            self.pos_embed = nn.Parameter(self.sinusoidal_embedding(num_patches + self.num_tokens, embed_dim),
                                            requires_grad=False)
        self.pos_drop = nn.Dropout(p=drop_rate)
        self.num_heads = num_heads
        self.use_head = use_head

        if args.return_attention is None:
            self.return_attention = [False for i in range(depth)]
        elif args.return_attention:
                self.return_attention = [True for i in range(depth)]
        else:
            self.return_attention = [False for i in range(depth)]

        if return_z is None:
            self.return_z = [False for i in range(depth)]
        else:
            self.return_z = [return_z[i] == "True" for i in range(depth)]
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                return_attention=self.return_attention[i],
                return_z=self.return_z[i],
                args=args,
                block_nr=i)
        ...
        if isinstance(num_classes, list):
            num_classes = num_classes[0] #hack to avoid errors
        self.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
        if args.dropout_head > 0:
            self.head = nn.Sequential(nn.Dropout(args.dropout_head), self.head)
        self.head_dist = None
        if distilled:
    ...
    def forward(self, x):
        ...
        if self.head_dist is not None:
            x['x'], x['x_dist'] = self.head(x['x'][0]), self.head_dist(x['x'][1])  # x must be a tuple
            if self.training and not torch.jit.is_scripting():
                # during inference, return the average of both classifier predictions
                return x['x'], x['x_dist']
            else:
                return (x['x'] + x['x_dist']) / 2
        elif self.use_head:
            x['x'] = self.head(x['x'])
        else:
            x['x'] = x['x']
